...
scores_link = "http://www.espn.com/mens-college-basketball/scoreboard/_/date/20" + str(sys.argv[1])
    ...
    if time in adjusted_times:
        ...
        for i in range(len(adjusted_times)):
            if time == adjusted_times[i] and i not in started_games:
                ...
                teams[i][0].replace(" ", "_")
                teams[i][1].replace(" ", "_")
                command = "python ncaa_stat_collection.py " + str(game_links[i]) + " " + str(teams[i][0]) + " " + str(teams[i][1]) + " " + sys.argv[1] + " &"
                os.system(command)
